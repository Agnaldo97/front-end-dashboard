<template>
  <div>
    ...
    <b-tabs v-model="tabIndex" class="nav-tabs-top mb-4">
          ...
          <table class="table">
            ...
            <tbody>
              <tr v-for="user in users" :key="user.id">
                ...
                <td><a @click="edit(user)">Editar</a> - <a @click="removeUser(user)">Excluir</a></td>
              </tr>
            </tbody>
          </table>
        </b-card>
        ...
        <b-card header="Usuários" header-tag="h6" class="mb-4">
          <b-form-row>
            <b-form-group label="Nome" class="col-md-3">
              <b-input type="text" v-validate="'required'" v-model="user.name" placeholder="Insira o nome" />
            </b-form-group>
            <b-form-group label="Telefone" class="col-md-3">
              <b-input type="text" v-validate="'required'" v-model="user.phone" placeholder="Insira o telefone" />
            </b-form-group>
            <b-form-group label="Sexo" class="col-md-3">
              <b-select value="Selecione o sexo" v-validate="'required'" v-model="user.gender" :options="genderOptions" />
            </b-form-group>
          ...
          <b-form-row>
            <b-form-group label="Email" class="col-md-3">
              <b-input type="email" v-validate="'required'" v-model="user.email" placeholder="Insira o email" />
            </b-form-group>
            <b-form-group label="Senha" class="col-md-3">
              <password v-model="user.password" v-validate="'required'" placeholder="Insira a senha" :secureLength="6" :badge="false" defaultClass="form-control" />
            </b-form-group>
          </b-form-row>
          <b-form-row id="btn-login-box">
            <b-btn v-if="!isEditing" id="btn-login" @click="cadaster()" variant="primary">Cadastrar</b-btn>
            <b-btn v-if="isEditing" id="btn-login" @click="edit()" variant="primary">Salvar</b-btn>
          </b-form-row>
        </b-card>
      </b-tab>
    </b-tabs>
